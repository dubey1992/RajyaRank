...
export default function ForgotPasswordPage() {
  ...
  async function requestReset() {
    if (!email.trim()) return setErrors({ email: L('कृपया अपना ईमेल पता दर्ज करें।', 'Please enter your email address.') });
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return setErrors({ email: L('कृपया मान्य ईमेल पता दर्ज करें।', 'Please enter a valid email address.') });
    setErrors({});
    setBusy(true);
    try {
      await apiFetch('/auth/student/password/forgot', { method: 'POST', body: JSON.stringify({ email: email.trim() }) });
      setStep('reset');
    } catch (e) {
  ...
  async function doReset() {
    ...
    const errs = validate(studentPasswordResetSchema, payload);
    setErrors(errs);
    if (Object.keys(errs).length) return;
    setBusy(true);
    try {
      await apiFetch('/auth/student/password/reset', { method: 'POST', body: JSON.stringify(payload) });
      setDone(true);
    } catch (e) {
      setErrors(serverFieldErrors(e as ApiError));
    } finally {
  ...
  return (
    <main id="main" className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-4">
      ...
      <p className="mb-6 text-sm text-muted">
        {L('अपना ईमेल दर्ज करें — हम आपको एक रीसेट कोड भेजेंगे।', 'Enter your email — we will send you a reset code.')}
      </p>
      ...
      {done ? (
        <div className="grid gap-4">
          ...
          <Link href={`/${locale}/login`} className="text-center text-sm font-extrabold text-navy-900 hover:underline">
      ...
      {!done && step === 'request' ? (
        <form noValidate onSubmit={(e) => { e.preventDefault(); void requestReset(); }}>
          <Field label={L('ईमेल', 'Email')} name="email" type="email" autoComplete="username" value={email} error={errors.email} onChange={(e) => setEmail(e.target.value)} />
      ...
      <Link href={`/${locale}/login`} className="mt-6 text-center text-sm text-muted hover:underline">
